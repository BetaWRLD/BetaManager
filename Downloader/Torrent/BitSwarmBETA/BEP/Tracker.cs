    ...
    internal class Tracker
    {
        ...
        static Tracker()
        {
            ...
            httpClient.DefaultRequestHeaders.UserAgent.Add(
                new System.Net.Http.Headers.ProductInfoHeaderValue("BitSwarm", BitSwarm.Version)
            );
        ...
        public Tracker(Uri url, Options options)
        {
            ...
            typeHostPort = (type.ToString().ToLower() + "://" + host + ":" + port).PadRight(
                50,
                ' '
            );
        }
        ...
        private void InitializeUDP()
        {
            ...
            try
            {
                ips = Dns.GetHostAddresses(uri.DnsSafeHost);
            }
            catch (Exception) { }
        ...
        )
        {
            try
            {
                ...
                query +=
                    $"info_hash={Utils.StringHexToUrlEncode(options.InfoHash)}&peer_id={Utils.StringHexToUrlEncode(BitConverter.ToString(options.PeerId).Replace("-", ""))}&port=11111&left={left}&downloaded={downloaded}&uploaded={uploaded}&event=started&compact=1&numwant={num_want}&key={key.ToString("x")}";
                ...
                {
                    IPAddress curIP = new IPAddress(
                        Utils.ArraySub(ref hashBytes, (uint)i, 4, false)
                    );
                    UInt16 curPort = (UInt16)
                        BitConverter.ToInt16(
                            Utils.ArraySub(ref hashBytes, (uint)i + 4, 2, true),
                            0
                        );
                    if (curPort > 0)
        ...
        )
        {
            try
            {
                ...
                if (num_want != -1 && curRecursions * 200 > num_want)
                    return;
                if (num_want == -1 && curRecursions > 3)
                    return;
                ...
                else
                {
                    ...
                    data = Utils.ArrayMerge(
                        connID,
                        action,
                        tranID,
                        Utils.StringHexToArray(options.InfoHash),
                        options.PeerId,
                        Utils.ToBigEndian(downloaded),
                        Utils.ToBigEndian(left),
                        Utils.ToBigEndian(uploaded),
                        Utils.ToBigEndian(event_),
                        Utils.ToBigEndian(externalIp),
                        Utils.ToBigEndian(key),
                        Utils.ToBigEndian(num_want),
                        Utils.ToBigEndian(externalPort)
                    );
                    udpClient.Send(data, data.Length, rEP);
        ...
        private void ReceiveUDP(IAsyncResult ar)
        {
            try
            {
                ...
                else if (Utils.ArrayComp(Utils.ToBigEndian((Int32)Action.ANNOUNCE), action))
                {
                    ...
                    {
                        IPAddress curIP = new IPAddress(
                            Utils.ArraySub(ref recvBuff, (uint)(20 + (i * 6)), 4, false)
                        );
                        UInt16 curPort = (UInt16)
                            BitConverter.ToInt16(
                                Utils.ArraySub(ref recvBuff, (uint)(24 + (i * 6)), 2, true),
                                0
                            );
        ...
        {
            if (options.Verbosity > 0)
                options.LogFile.Write($"[Tracker ] [{typeHostPort}] {msg}");
        }
    }
}
