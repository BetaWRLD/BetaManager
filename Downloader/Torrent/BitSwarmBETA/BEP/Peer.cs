...
namespace SuRGeoNix.BitSwarmLib.BEP
{
    class Peer
    {
        ...
        public static readonly byte[] BIT_PROTO = Utils.ArrayMerge(
            new byte[] { 0x13 },
            Encoding.UTF8.GetBytes("BitTorrent protocol")
        );
        public static readonly byte[] EXT_PROTO = Utils.ArrayMerge(
            new byte[] { 0, 0, 0, 0 },
            new byte[] { 0, 0x10, 0, (0x1 | 0x4) }
        );
        ...
        public void RunNext(int available = -1)
        {
            try
            {
                ...
                if (Beggar.torrent?.file?.paths?.Count > 0)
                {
                    Dispose();
                    return;
                }
                ...
                if (waitingFor != 0)
                {
                    ...
                    if (available < waitingFor)
                    {
                        ...
                        {
                            if (
                                Beggar.Stats.CurrentTime - lastRequest
                                > Beggar.Options.PieceTimeout
                            ) // Extended Handshake timeout TBR
                                Dispose();
                        }
                        else if (status == Status.DOWNLOADING)
                        {
                            long curTimeout =
                                Beggar.FocusAreInUse && Beggar.Options.EnableBuffering
                                    ? Beggar.Options.PieceBufferTimeout * 10000
                                    : Beggar.Options.PieceTimeout * 10000;
                            ...
                            if (Beggar.Stats.CurrentTime - blockRequestedAt > curTimeout)
                            {
                                ...
                                int curRetries =
                                    Beggar.FocusAreInUse && Beggar.Options.EnableBuffering
                                        ? Beggar.Options.PieceBufferRetries
                                        : Beggar.Options.PieceRetries;
                                bool bufferTimeoutUsed =
                                    Beggar.Options.EnableBuffering && Beggar.FocusAreInUse;

                                if (
                                    PieceTimeouts == 1
                                    && (
                                        (bufferTimeoutUsed && Beggar.Options.PieceBufferRetries > 0)
                                        || (!bufferTimeoutUsed && Beggar.Options.PieceRetries > 0)
                                    )
                                    && lastPieces != null
                                    && lastPieces.Count > 0
                                )
                                ...
                                if (Beggar.Options.Verbosity > 0)
                                    Log(
                                        4,
                                        $"[TIMEOUT] {PieceTimeouts} ({tcpClient.Client.Available} < {waitingFor} , Requests: {PiecesRequested}, Pieces: {lastPieces.Count}, Timeouts: {PieceTimeouts})"
                                    );
                                ...
                                if (PieceTimeouts > curRetries)
                                {
                                    if (Beggar.Options.Verbosity > 0)
                                        Log(
                                            3,
                                            $"[DROP] Piece Timeout ({tcpClient.Client.Available} < {waitingFor} , Requests: {PiecesRequested}, Pieces: {lastPieces.Count}, Timeouts: {PieceTimeouts})"
                                        );
                ...
                switch (curStep)
                {
                    ...
                    case 2:
                        if (taskConnect.IsCompleted)
                        {
                            ...
                            if (tcpClient.Connected)
                            {
                                ...
                                tcpStream.Write(
                                    Beggar.HANDSHAKE_BYTES,
                                    0,
                                    Beggar.HANDSHAKE_BYTES.Length
                                );
                        ...
                        {
                            if (Beggar.Stats.CurrentTime - lastRequest > curTimeout)
                            {
                                Beggar.Stats.PeersConnecting--;
                                Dispose();
                            }
                    ...
                    case 4:
                        ...
                        {
                            if (Beggar.torrent.metadata.isDone != null)
                            {
                                if (
                                    Beggar.OptionsClone.MetadataParallelReq > 0
                                    && stageYou.extensions.ut_metadata != 0
                                    && stageYou.metadataPiecesRequested == 0
                                    && lastPieces.Count == 0
                                )
                                {
                                    stageYou.metadataRequested = false;
                        ...
                        waitingFor =
                            waitingFor < 0
                                ? -1
                                : (waitingFor > MAX_DATA_SIZE * 2 ? -1 : waitingFor); // MAX_DATA_SIZE + X + 1 + 4?
                        ...
                        if (waitingFor == -1)
                        {
                            if (Beggar.Options.Verbosity > 0)
                                Log(
                                    1,
                                    $"Invalid wait for {Utils.ToBigEndian(Beggar.recvBuff)} bytes"
                                );
                            Dispose();
                    ...
                    case 5:
                        ...
                        if (status == Status.READY && available == 0) // TBR: 0 Data to avoid Requesting while choke pending
                        {
                            ...
                            if (Beggar.torrent?.metadata.isDone == true || Beggar.torrent == null)
                                return;
                            Beggar.RequestPiece(this);
                        }
                        ...
                        if (Beggar.torrent?.metadata.isDone == true)
                            return;
                        RunNext(available);
        ...
        private void ProcessMessage(int msgId, int msgLen)
        {
            ...
            switch (msgId)
            {
                ...
                case Messages.REJECT_REQUEST: // Piece | Offset | Length
                    ...
                    if (lastPiece != -1)
                    {
                        Beggar.ResetRequest(
                            ...
                            piece,
                            offset,
                            Utils.ToBigEndian(Beggar.recvBuff, 8)
                        );
                        lastPieces.RemoveAt(lastPiece);
                    }
                ...
                case Messages.BITFIELD: // Bitfield
                    ...
                    stageYou.bitfield = new Bitfield(
                        bitfield,
                        Beggar.torrent.data.pieces != 0 ? Beggar.torrent.data.pieces : msgLen * 8
                    );
        ...
        private void ProcessMessageExt(int msgId, int msgLen)
        {
            ...
            if (msgId == Messages.EXTENDED_HANDSHAKE)
            {
                ...
                if (cur != null)
                    stageYou.extensions.ut_metadata = (byte)((int)cur);
            ...
            else if (msgId == Messages.EXT_UT_PEX)
            {
                ...
                if (extDic.ContainsKey("added"))
                    buffAdded = ((BString)extDic["added"]).Value.ToArray();

                for (int i = 0; i < buffAdded.Length / 6; i++)
                {
                    System.Net.IPAddress curIP = new System.Net.IPAddress(
                        Utils.ArraySub(ref buffAdded, (uint)i * 6, 4, false)
                    );
                    UInt16 curPort = (UInt16)
                        BitConverter.ToInt16(
                            Utils.ArraySub(ref buffAdded, (uint)4 + (i * 6), 2, true),
                            0
                        );
                ...
                if (extDic.ContainsKey("added6"))
                    buffAdded = ((BString)extDic["added6"]).Value.ToArray();

                for (int i = 0; i < buffAdded.Length / 18; i++)
                {
                    System.Net.IPAddress curIP = new System.Net.IPAddress(
                        Utils.ArraySub(ref buffAdded, (uint)i * 18, 16, false)
                    );
                    UInt16 curPort = (UInt16)
                        BitConverter.ToInt16(
                            Utils.ArraySub(ref buffAdded, (uint)16 + (i * 18), 2, true),
                            0
                        );
                ...
                if (peers.Count > 0)
                    Beggar.FillPeers(peers, BitSwarm.PeersStorage.PEX);
            ...
            else if (msgId == Messages.EXT_UT_METADATA)
            {
                ...
                switch (mdHeadersDic.Get<BNumber>("msg_type").Value)
                {
                    case Messages.METADATA_RESPONSE: // (Expecting 0x4000 | 16384 bytes - except if last piece)
                        ...
                        Beggar.MetadataPieceReceived(
                            Beggar.recvBuff,
                            (int)mdHeadersDic.Get<BNumber>("piece").Value,
                            mdHeadersDic.EncodeAsString().Length,
                            (int)mdHeadersDic.Get<BNumber>("total_size").Value,
                            msgLen - mdHeadersDic.EncodeAsString().Length,
                            this
                        );
        ...
        private void SendExtendedHandshake()
        {
            ...
            {
                if (Beggar.Options.Verbosity > 0)
                    Log(1, "[HAND] Extended Sending Error " + e.Message);
                Dispose();
            }
        }
        ...
        public void Dispose()
        {
            try
            {
                if (
                    lastPieces != null
                    && lastPieces.Count > 0 /*&& Beggar.isRunning*/
                )
                    Beggar.ResetRequests(this, lastPieces);
        ...
        public void RequestMetadata(int piece, int piece2 = -1)
        {
            try
            {
                ...
                sendBuff = PrepareMessage(
                    stageYou.extensions.ut_metadata,
                    true,
                    Encoding.UTF8.GetBytes(
                        (new BDictionary { { "msg_type", 0 }, { "piece", piece } }).EncodeAsString()
                    )
                );
                if (piece2 != -1)
                    sendBuff = Utils.ArrayMerge(
                        sendBuff,
                        PrepareMessage(
                            stageYou.extensions.ut_metadata,
                            true,
                            Encoding.UTF8.GetBytes(
                                (
                                    new BDictionary { { "msg_type", 0 }, { "piece", piece2 } }
                                ).EncodeAsString()
                            )
                        )
                    );
        ...
        public static byte[] PrepareMessage(byte msgid, bool isExtended, byte[] payload)
        {
            ...
            if (isExtended)
            {
                ...
                if (payload != null)
                    Buffer.BlockCopy(payload, 0, tmp, 6, payload.Length);
            ...
            else
            {
                ...
                if (payload != null)
                    Buffer.BlockCopy(payload, 0, tmp, 5, payload.Length);
        ...
        private int FindPiece(int piece, int offset)
        {
            for (int i = 0; i < lastPieces.Count; i++)
                if (lastPieces[i].Item1 == piece && lastPieces[i].Item2 == offset)
                    return i;

            return -1;
        }

        internal void Log(int level, string msg)
        {
            if (
                Beggar.Options.Verbosity > 0
                && Beggar.Options.LogPeer
                && level <= Beggar.Options.Verbosity
            )
                Beggar.log.Write($"[Peer    ] [{host.PadRight(15, ' ')}] {msg}");
        }
        #endregion
    }
}
