    ...
    public class BitStream : Stream
    {
        ...
        public override int Read(byte[] buffer, int offset, int count)
        {
            ...
            while (tempPos < this.Position + offset + count && tempPos < this.Length)
            {
                ...
                {
                    buffer[readPosCount] = (byte)(
                        (int)(buffer[readPosCount]) | (0x1 << (7 - readPosMod))
                    );
                }
                else
                {
                    buffer[readPosCount] = (byte)(
                        (int)(buffer[readPosCount]) & (0xffffffff - (0x1 << (7 - readPosMod)))
                    );
                }
        ...
        public override void Write(byte[] buffer, int offset, int count)
        {
            ...
            while (tempPos < this.Position + count && tempPos < this.Length)
            {
                ...
                {
                    this.Source[posCount] = (byte)(
                        (int)(this.Source[posCount]) | (0x1 << (7 - posMod))
                    );
                }
                else
                {
                    this.Source[posCount] = (byte)(
                        (int)(this.Source[posCount]) & (0xffffffff - (0x1 << (7 - posMod)))
                    );
                }
